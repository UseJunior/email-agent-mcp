...
- Independent of issue #93. `ctx.allMailboxes` is available today, and the round-trip guarantee is asserted directly against action dispatch rather than through `list_mailboxes`. #93 has since shipped that MCP tool (`buildLazyActions()` registers it as of `238f5d2`), but this change deliberately does not route the round trip through it: the scenario verifies that a name from the error payload *selects a mailbox on dispatch*, which is what `resolveMailboxContext` does — enumeration via `list_mailboxes` is a separate concern.
